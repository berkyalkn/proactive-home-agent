...
export default function Step3Biometrics({ onNext }: Props) {
  ...
  return (
    <div className="bg-white border border-slate-200 p-10 rounded-[2rem] shadow-xl shadow-slate-200/50 text-center min-h-[500px] flex flex-col items-center justify-center transform-gpu relative overflow-hidden">
      ...
      <AnimatePresence mode="wait">
        ...
        {mode === 'intro' && (
          <motion.div key="intro" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="w-full relative z-10">
            ...
            <div className="space-y-3 w-full max-w-xs mx-auto">
              <button onClick={startFaceSetup} className="w-full py-4 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-2">
                <Sparkles className="w-5 h-5" /> Let's Get Started
              </button>
              <button onClick={handleSkip} className="w-full py-4 bg-white hover:bg-slate-50 text-slate-500 font-bold rounded-xl transition-all">
                Maybe Later
              </button>
            </div>
          </motion.div>
        ...
        {mode === 'face' && (
          <motion.div key="face" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col items-center w-full relative z-10">
            ...
            {currentFaceStep !== "done" && (
              <button onClick={capturePhoto} className="px-8 py-4 bg-slate-900 hover:bg-slate-800 text-white font-bold rounded-xl shadow-lg transition-all flex items-center gap-2">
        ...
        {mode === 'voice' && (
          <motion.div key="voice" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col items-center w-full relative z-10">
            ...
            {isRecording ? (
              <button onClick={stopVoiceRecording} className="px-8 py-4 font-bold rounded-xl shadow-lg transition-all flex items-center gap-2 bg-indigo-600 hover:bg-indigo-700 text-white animate-pulse">
                <StopCircle className="w-5 h-5" /> Finish & Save
              </button>
            ) : (
              <button onClick={startVoiceRecording} className="px-8 py-4 font-bold rounded-xl shadow-lg transition-all flex items-center gap-2 bg-slate-900 hover:bg-slate-800 text-white">
