...
import { ShieldAlert, Phone, User, MessageSquare, PhoneCall, Send, Bot, Settings2, ArrowRight, Loader2 } from 'lucide-react';
          ...
          <motion.div key="setup" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="w-full relative z-10 flex flex-col h-full">
            ...
            <div className="flex-1 flex flex-col w-full h-[480px]">
                ...
                {loading ? (
                    ...
                    <div className="flex-1 overflow-y-auto scrollbar-hide text-left space-y-5 pr-2 pb-4">
                        ...
                        <div className="space-y-2">
                            ...
                            <div className="grid grid-cols-1 gap-2">
                                <label className={`flex items-center p-3 rounded-xl border cursor-pointer transition-all ${security.use_sms ? "bg-emerald-50 border-emerald-200 shadow-sm" : "bg-slate-50 border-slate-200"}`}>
                                ...
                                <label className={`flex items-center p-3 rounded-xl border cursor-pointer transition-all ${security.use_voice_call ? "bg-emerald-50 border-emerald-200 shadow-sm" : "bg-slate-50 border-slate-200"}`}>
                                ...
                                <label className={`flex items-center p-3 rounded-xl border cursor-pointer transition-all ${security.use_telegram ? "bg-sky-50 border-sky-200 shadow-sm" : "bg-slate-50 border-slate-200"}`}>
                        ...
                        <div className="flex flex-col pt-1">
                            <label className="text-[10px] font-extrabold text-slate-400 ml-1 mb-1.5 flex items-center gap-1 tracking-widest uppercase"><Bot className="w-3 h-3"/> AI System Response</label>
                            <textarea 
                                rows={2} placeholder="What should the house do?"
                                value={security.emergency_action_text} onChange={(e) => handleChange("emergency_action_text", e.target.value)}
                                className="w-full px-4 py-3 rounded-xl bg-slate-50 border border-slate-200 focus:border-rose-400 outline-none transition-colors text-sm font-medium text-slate-700 resize-none"
                            />
                        </div>
                    </div>
                )}
            ...
            <div className="mt-4 pt-6 border-t border-slate-100 flex gap-3 shrink-0 w-full">
                ...
                <button 
                    ...
                    disabled={saving} 
                    className="flex-1 py-3.5 bg-rose-600 hover:bg-rose-700 text-white font-bold rounded-xl shadow-md transition-all flex items-center justify-center gap-2"
                >
